...
{
	const auto it = vertexToIndexMap.find(vertex);
							  ...
							  std::vector<XMFLOAT3>& outNormals)
{
	...
	std::map<SimpleVertex, unsigned short> vertexToIndexMap;

	std::pair<SimpleVertex, unsigned short> pair;

	const int numVertices = inVertices.size();
	...
	for(int i = 0; i < numVertices; ++i) //For each vertex
	{
		...
		const bool found = FindSimilarVertex(vertex, vertexToIndexMap, index); 
...
MeshData OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if(!binaryInFile.good())
	{
		...
		while(!inFile.eof()) //While we have yet to reach the end of the file...
		{
			...
			else if(input.compare("f") == 0) //Face
			{
				for(int i = 0; i < 3; ++i)
				{
					...
					//Parse into int
					//atoi = "ASCII to int"
					vInd[i] = static_cast<unsigned short>(atoi(beforeFirstSlash.c_str()));
				...
				{
					vertexIndices.push_back(vInd[i] - 1);		//Minus 1 from each as these as OBJ indexes start from 1 whereas C++ arrays start from 0
		...
		unsigned int numIndices = vertexIndices.size();
		for(unsigned int i = 0; i < numIndices; i++)
		{
			expandedVertices.push_back(vertices[vertexIndices[i]]);
		...
		auto finalVertices = new SimpleVertex[meshVertices.size()];
		unsigned int numMeshVertices = meshVertices.size();
		for(unsigned int i = 0; i < numMeshVertices; ++i)
		{
			finalVertices[i].Pos = meshVertices[i];
			finalVertices[i].Normal = meshNormals[i];
			finalVertices[i].TexC = meshTexCoords[i];
		}
		...
		outbin.write(reinterpret_cast<char*>(finalVertices), sizeof(SimpleVertex) * numMeshVertices);
	...
	else
	{
		...
		//Read in array sizes
		binaryInFile.read(reinterpret_cast<char*>(&numVertices), sizeof(unsigned int));
		binaryInFile.read(reinterpret_cast<char*>(&numIndices), sizeof(unsigned int));
		...
		binaryInFile.read(reinterpret_cast<char*>(finalVerts), sizeof(SimpleVertex) * numVertices);
		binaryInFile.read(reinterpret_cast<char*>(indices), sizeof(unsigned short) * numIndices);
